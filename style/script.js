...
document.addEventListener('mousemove', e => {
    ...
    if (delta > 0 && delta < 360) {
        flipCard.querySelector('.flip-card-back').style.transform =
            `rotateY(180deg) translateY(${delta}px)`;
    }
...
document.addEventListener('mouseup', () => {
    isDragging = false;
    flipCard.querySelector('.flip-card-back').style.transform = 'rotateY(180deg)';
});
